...
namespace Commercers\AutoProductRelation\Block\Cart;
...
class Crosssell extends \Magento\Catalog\Block\Product\AbstractProduct {
     ...
     * @param \Commercers\AutoProductRelation\Model\Services\CrossSell\SendEmailWithCrossSellProduct $sendEmailWithCrossSellProduct
    ...
    public function getReferralCode(){
        $couponCode = $this->checkApplyCouponCode();
        $crossSellFollow = $this->crossSellFollowFactory->create()->load($couponCode,'promotion_code');
        return $crossSellFollow->getReferralCode();
    }
    public function checkApplyCouponCode(){
        $getCouponObj = $this->getLayout()
        ->createBlock('Magento\Checkout\Block\Cart\Coupon');
        return $getCouponObj->getCouponCode(); 
    }
    public function getCrossSellProduct() {
//        $params = $this->getRequest()->getParam('params');
//        if($params){
//            $uriDecode = base64_decode($params);
//            $explodeUri = explode('/', $uriDecode);
//            $referralCode = $explodeUri[3];
//            $crossSellFollow = $this->crossSellFollowFactory->create()->load($referralCode,'referral_code');
//            $couponCode = $crossSellFollow->getPromotionCode();
        $couponCode = $this->checkApplyCouponCode();
        $items = array();
        if ($couponCode) {
            $productRecommendation = $this->productRecommendation->create()->load($couponCode, 'coupon_code');
            $jsonArray = json_decode($productRecommendation['product_linked']);
            $jsonDecode = get_object_vars($jsonArray);
            foreach ($jsonDecode['product_id'] as $value) {
                $productIds[] = $value;
            }
            $productAddCarts = $this->_getCartProductIds();
            $selectedProductIds = array();
            foreach ($productIds as $productId) {
                if (!in_array($productId, $productAddCarts)) {
                    $selectedProductIds[] = $productId;
                }
            }
            $productId = implode(',', $selectedProductIds);
            $items = $this->collectionFactory->create()->addFieldToFilter('entity_id', array('in' => $productId))->addAttributeToSelect('*');
...
//        }
    ...
    public function getParentId($productId){
        ...
        $product = $this->_productFactory->create()->load($parentId);
     ...
     * @return array
     */
    public function _getCartProductIds() {
